...
console.log(carYears.sort(function(a, b) {return a - b;}) );
...
let small =[]
for(let i = 0;i < inventory.length;i++){
    small.push(inventory[i].car_make)
}
console.log(small.sort());

for(let i = 0;i < 5;i++){
    BMWAndAudi.push(small[i])
    
}
let myJSON = JSON.stringify(BMWAndAudi);
console.log(BMWAndAudi,myJSON);
